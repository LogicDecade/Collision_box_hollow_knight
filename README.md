...
**移动端触屏**（`pointer: coarse` 设备自动启用；建议横屏，竖屏会全屏提示旋转）

- 左半屏**虚拟摇杆**：左右推动移动（固定速度，非无极变速）；**摇杆上 / 下 = 上劈 / 下劈**，水平或居中 = 平砍
- 右下按钮：`魂`（按住回血）/ `攻` / `跳`（长按跳高）；右上 `❚❚` 暂停
- 按钮区已避开 iOS Safari 底部安全区（`env(safe-area-inset-*)`）
